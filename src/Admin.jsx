    ...
    const fetchProducts = () => {
        ...
        .then(r => r.json())
        .then(d => { if (d.success) { setProducts(d.data); setServerError(false); } })
...
fetch(`https://rayfinesite-3.onrender.com/api/products/${id}`, { method: "DELETE" })
    ...
    const chartData = orders.reduce((acc, o) => {
        const date = o._id?.toString().substring(0, 8) || "N/A";
    ...
    return (
        ...
        <div style={{ flex: 1, padding: "40px", overflowY: "auto" }}>
            ...
            <div style={{ background: "#ff444422", border: "1px solid #ff4444", borderRadius: "10px", padding: "14px 20px", marginBottom: "24px", color: "#ff8888" }}>
                ⚠️ Server se connect nahi ho raha — backend start karo
            </div>
            )}
            ...
            <div>
                ...
                {["name", "price", "image", "description"].map(field => (
                    field === "description"
                    ? <textarea key={field} name={field} placeholder="Description" value={form[field]} onChange={handleChange} style={{ ...inputStyle, height: "80px" }} />
                    : <input key={field} type={field === "price" ? "number" : "text"} name={field}
                        placeholder={field.charAt(0).toUpperCase() + field.slice(1)}
                        value={form[field]} onChange={handleChange} style={inputStyle} />
                ))}
                {form.image && <img src={form.image} alt="preview" style={{ width: "100%", height: "160px", objectFit: "cover", borderRadius: "10px", marginBottom: "10px" }} onError={e => e.target.style.display = "none"} />}
                ...
                {filteredProducts.map(p => (
                    <div key={p._id} style={{ background: "#111", borderRadius: "12px", border: "1px solid #333", overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "12px" }}>
                        <p style={{ fontWeight: "bold", marginBottom: "4px" }}>{p.name}</p>
                        <p style={{ color: "#F9CDC2", marginBottom: "10px" }}>₹{p.price}</p>
                        <button onClick={() => deleteProduct(p._id)} style={{ width: "100%", padding: "8px", background: "#ff4444", border: "none", borderRadius: "8px", color: "#fff", cursor: "pointer" }}>
                        🗑 Delete
                        </button>
                    </div>
                    </div>
            ...
            <div>
                ...
                {orders.length === 0 ? <p style={{ color: "#888" }}>Abhi tak koi order nahi</p>
                : orders.map((o, i) => (
                    ...
                    <p style={{ color: "#888", fontSize: "13px", marginTop: "8px" }}>Items: {o.items?.map(item => item.name).join(", ") || "N/A"}</p>
                    </div>
                ))}
            </div>
            )}
            ...
            <div>
                ...
                {chartData.length === 0 ? <p style={{ color: "#888" }}>Orders aane ke baad chart dikhega</p>
