...
export default function Admin() {
  ...
  if (!authed) {
    return (
      <div style={{ minHeight: "100vh", background: "var(--cream)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',sans-serif" }}>
        <div style={{ background: "#fff", border: "1px solid var(--border-light)", borderRadius: 12, padding: "48px 44px", maxWidth: 400, width: "100%", textAlign: "center", boxShadow: "0 20px 60px rgba(44,36,24,0.1)" }}>
          <div style={{ fontSize: 32, marginBottom: 10 }}>◈</div>
          <h2 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 28, fontWeight: 400, color: "var(--text)", marginBottom: 6 }}>Admin Access</h2>
          <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 28, letterSpacing: "0.5px" }}>Ray Fine Ornates · Store Management</p>
          <input
            type="password"
            ...
            style={{ width: "100%", boxSizing: "border-box", padding: "12px 16px", border: "1.5px solid var(--border)", borderRadius: 6, fontSize: 14, fontFamily: "inherit", outline: "none", marginBottom: 8, color: "var(--text)" }}
          />
          ...
          <button
            ...
            style={{ width: "100%", padding: "12px", background: "var(--text)", color: "#fff", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}
          >
            Login →
          </button>
  ...
  const card = { background: "#fff", border: "1px solid var(--border-light)", borderRadius: 10, padding: "20px 22px", marginBottom: 16 };
  const inputSt = { width: "100%", boxSizing: "border-box", padding: "9px 12px", border: "1.5px solid var(--border)", borderRadius: 6, fontSize: 13, fontFamily: "'DM Sans',sans-serif", outline: "none", color: "var(--text)", background: "#fff" };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", fontFamily: "'DM Sans',sans-serif", background: "var(--cream)" }}>
      <style>{`
        ...
        .admin-nav-item:hover { background: #f5ede4 !important; }
        ...
        .admin-table-row:hover td { background: #faf7f2; }
        .admin-btn-icon:hover { background: #f5ede4 !important; }
      ...
      {/* SIDEBAR */}
      <div style={{ width: sideOpen ? 224 : 60, minWidth: sideOpen ? 224 : 60, background: "#fff", borderRight: "1px solid var(--border-light)", display: "flex", flexDirection: "column", transition: "width 0.25s ease", overflow: "hidden", zIndex: 100, flexShrink: 0 }}>
        <div style={{ padding: "18px 16px 14px", borderBottom: "1px solid var(--border-light)", display: "flex", alignItems: "center", gap: 10, justifyContent: sideOpen ? "flex-start" : "center" }}>
          ...
          {sideOpen && (
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text)", letterSpacing: "0.5px", fontFamily: "Cormorant Garamond, serif" }}>Ray Fine Admin</div>
        ...
        <nav style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
          {NAV.map(n => (
            <div
              ...
              style={{ display: "flex", alignItems: "center", gap: 10, padding: sideOpen ? "9px 18px" : "9px 0", justifyContent: sideOpen ? "flex-start" : "center", cursor: "pointer", transition: "all .15s", color: page === n.id ? "var(--primary)" : "var(--text-muted)", fontSize: 13, borderLeft: page === n.id ? "3px solid var(--primary)" : "3px solid transparent" }}
            >
        ...
        <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border-light)", display: "flex", justifyContent: sideOpen ? "flex-end" : "center" }}>
          <button onClick={() => setSideOpen(!sideOpen)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "var(--text-muted)", padding: "4px 8px", borderRadius: 4 }}>
            {sideOpen ? "◀" : "▶"}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        {/* Topbar */}
        <div style={{ background: "#fff", borderBottom: "1px solid var(--border-light)", padding: "0 24px", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={() => setPage("upload")} style={{ padding: "7px 16px", borderRadius: 4, border: "none", background: "var(--text)", color: "#fff", fontSize: 11, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", cursor: "pointer" }}>
              + Add Product
            </button>
            <button
              onClick={() => { sessionStorage.removeItem("rfo_admin"); setAuthed(false); }}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
          {page === "dashboard" && <DashboardPage products={products} orders={orders} loading={loading} setPage={setPage} />}
          {page === "products" && (
            <ProductsPage
              ...
              setPage={setPage}
              showToast={showToast}
              onToggleSale={id => { setProducts(prev => prev.map(p => p.id === id ? { ...p, onSale: !p.onSale } : p)); showToast("Sale status updated"); }}
              onToggleStock={id => { setProducts(prev => prev.map(p => p.id === id ? { ...p, inStock: !p.inStock } : p)); showToast("Stock status updated"); }}
          ...
          {page === "bulk" && (
            <BulkUploadPage
              card={card} inputSt={inputSt} showToast={showToast}
              onImport={imported => {
          ...
          {page === "categories"  && <CategoriesPage card={card} showToast={showToast} />}
          {page === "collections" && <CollectionsPage products={products} setProducts={setProducts} card={card} showToast={showToast} />}
          {page === "orders"      && <OrdersPage orders={orders} setOrders={setOrders} card={card} showToast={showToast} />}
          {page === "notifications" && <NotificationsPage notifs={notifs} setNotifs={setNotifs} card={card} />}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid var(--border-light)", borderRadius: 10, padding: "18px 20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <button onClick={() => setPage("orders")} style={{ background: "none", border: "none", color: "var(--primary)", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>View all →</button>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {orders.slice(0, 5).map(o => (
                <tr key={o.id}>
                  <td style={{ padding: "8px", fontSize: 13, color: "var(--text-muted)" }}>{o.id}</td>
                  <td style={{ padding: "8px", fontSize: 13, color: "var(--text)" }}>{o.customer}</td>
                  <td style={{ padding: "8px", fontSize: 13, fontWeight: 500 }}>{formatINR(o.amount)}</td>
...
function ProductsPage({ products, loading, searchQ, setSearchQ, catFilter, setCatFilter, setPage, onToggleSale, onToggleStock, card, inputSt }) {
  function stockBadge(p) {
    ...
    if ((p.stock || 0) <= 5 && (p.stock || 0) > 0) return <Badge type="low">Low Stock</Badge>;
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        />
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)} style={{ ...inputSt, width: 160 }}>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        {loading ? (
          ...
          <div style={{ padding: "70px 20px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
            ...
            <p style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 24, fontWeight: 400, color: "var(--text)", margin: 0 }}>No products yet</p>
            <p style={{ fontSize: 13, color: "var(--text-muted)", maxWidth: 280, lineHeight: 1.7, margin: 0 }}>Your collection is empty. Add products to get started.</p>
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              <button onClick={() => setPage("upload")} style={{ padding: "10px 22px", background: "var(--text)", color: "#fff", border: "none", borderRadius: 5, fontSize: 11, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer" }}>+ Add Product</button>
              <button onClick={() => setPage("bulk")}   style={{ padding: "10px 22px", background: "transparent", color: "var(--text-muted)", border: "1px solid var(--border)", borderRadius: 5, fontSize: 11, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer" }}>Bulk Upload</button>
            </div>
          </div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
              ...
              <tbody>
                {products.map(p => (
                  <tr key={p.id} className="admin-table-row" style={{ borderBottom: "1px solid var(--border-light)" }}>
                    <td style={{ padding: "10px 14px" }}>
                      <img src={p.image || "https://placehold.co/44x44?text=?"} alt={p.name}
                    ...
                    <td style={{ padding: "10px 14px", maxWidth: 180 }}>
                      <div style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, color: "var(--text)" }}>{p.name}</div>
                    ...
                    <td style={{ padding: "10px 14px", fontSize: 13, fontWeight: 500, whiteSpace: "nowrap" }}>
                      {formatINR(p.price)}
                    ...
                    <td style={{ padding: "10px 14px" }}>
                      <div style={{ display: "flex", gap: 5 }}>
                        <button className="admin-btn-icon" onClick={() => onToggleStock(p.id)} title={p.inStock ? "Mark Out of Stock" : "Mark In Stock"}
                          style={{ padding: "5px 8px", border: "1px solid var(--border-light)", borderRadius: 5, background: "transparent", cursor: "pointer", fontSize: 13 }}>
                          {p.inStock ? "📦" : "🚫"}
                        </button>
                        <button className="admin-btn-icon" onClick={() => onToggleSale(p.id)} title="Toggle Sale"
                          style={{ padding: "5px 8px", border: "1px solid var(--border-light)", borderRadius: 5, background: "transparent", cursor: "pointer", fontSize: 13 }}>
                          🏷
                        </button>
                      </div>
...
function UploadPage({ onSave, card, inputSt, labelSt }) {
  ...
  const Field = ({ label, error, children, full }) => (
    <div style={full ? { gridColumn: "1/-1" } : {}}>
      <label style={labelSt}>
        {label}
        {error && <span style={{ color: "#A32D2D", marginLeft: 6, fontWeight: 700 }}>{error}</span>}
      </label>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      <div style={card}>
        <h3 style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 20, fontWeight: 400, color: "var(--text)", marginBottom: 20 }}>Add New Product</h3>
        <Row>
        ...
        <Row>
          <Field label="Image URL" full>
            <input style={inputSt} placeholder="https://…"
              value={form.image}
              onChange={e => { set("image", e.target.value); setImgPreview(e.target.value || null); }} />
          </Field>
        </Row>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={() => { setForm(blankProduct()); setErrors({}); setImgPreview(null); }}
...
function BulkUploadPage({ card, inputSt, showToast, onImport }) {
  ...
  const handleFile = f => {
    ...
    reader.onload = e => {
      ...
      } catch (err) { showToast("CSV format error", "error"); }
    };
    reader.readAsText(f);
  };
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      <div style={card}>
        <h3 style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 20, fontWeight: 400, color: "var(--text)", marginBottom: 8 }}>Bulk Upload via CSV</h3>
        <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 18, lineHeight: 1.6 }}>Upload multiple products at once. Download the template, fill in your products, then upload.</p>
        ...
        <div
          ...
          style={{ border: `2px dashed ${dragging ? "var(--primary)" : "var(--border)"}`, borderRadius: 8, padding: "36px 24px", textAlign: "center", cursor: "pointer", background: dragging ? "#fdf8f4" : "transparent", transition: "all .2s", marginBottom: 16 }}
        >
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>Only .csv files supported</p>
        </div>
        ...
        <div style={{ background: "var(--cream)", padding: "14px 16px", borderRadius: 6, fontSize: 12, color: "var(--text-muted)", marginBottom: 16, lineHeight: 1.7 }}>
          <strong style={{ color: "var(--text)" }}>Required CSV columns:</strong><br />{CSV_HEADERS}
        </div>
  ...
  const Chip = ({ item, onToggle }) => (
    <div onClick={onToggle}
      style={{
        ...
        background: item.on ? "#FDF8F4" : "transparent",
    ...
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 16 }}>
          {occs.map((o, i) => (
            <Chip key={o.name} item={o} onToggle={() => setOccs(prev => prev.map((x, j) => j === i ? { ...x, on: !x.on } : x))} />
          ))}
        </div>
      ...
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 16 }}>
          {cats.map((c, i) => (
            <Chip key={c.name} item={c} onToggle={() => setCats(prev => prev.map((x, j) => j === i ? { ...x, on: !x.on } : x))} />
          ))}
        </div>
                  ...
                  <tr key={p.id} className="admin-table-row" style={{ borderBottom: "1px solid var(--border-light)" }}>
                    <td style={{ padding: "9px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <img src={p.image || "https://placehold.co/36x36?text=?"} alt={p.name}
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, marginBottom: 16 }}>
        {[
          ["all",       "All",       orders.length,                                   "var(--text)"],
          ["new",       "New",       orders.filter(o => o.status === "new").length,        "#A32D2D"],
          ["processing","Processing",orders.filter(o => o.status === "processing").length, "#854F0B"],
          ["shipped",   "Shipped",   orders.filter(o => o.status === "shipped").length,    "#0F6E56"],
          ["delivered", "Delivered", orders.filter(o => o.status === "delivered").length,  "#3B6D11"],
        ].map(([key, label, value, color]) => (
          <div key={key} onClick={() => setFilter(key)}
            style={{ background: "#fff", border: `1px solid ${filter === key ? "var(--primary)" : "var(--border-light)"}`, borderRadius: 8, padding: "12px 14px", cursor: "pointer", transition: "all .15s" }}>
            <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 700, letterSpacing: "0.8px", textTransform: "uppercase", marginBottom: 5 }}>{label}</div>
            <div style={{ fontSize: 22, fontWeight: 500, color, fontFamily: "Cormorant Garamond,serif" }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        <input placeholder="Search by order ID or customer…"
          ...
          style={{ flex: 1, padding: "9px 12px", border: "1.5px solid var(--border)", borderRadius: 6, fontSize: 13, fontFamily: "'DM Sans',sans-serif", outline: "none", color: "var(--text)", background: "#fff" }} />
        <span style={{ alignSelf: "center", fontSize: 12, color: "var(--text-muted)", whiteSpace: "nowrap" }}>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
            ...
            <tbody>
              {filtered.map(o => (
                <tr key={o.id} className="admin-table-row" style={{ borderBottom: "1px solid var(--border-light)" }}>
                  <td style={{ padding: "10px 14px", fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{o.id}</td>
                  ...
                  <td style={{ padding: "10px 14px", fontSize: 13, fontWeight: 500 }}>{formatINR(o.amount)}</td>
                  <td style={{ padding: "10px 14px" }}>
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto" }}>
      <div style={card}>
        ...
        {notifs.map(n => (
          <div key={n.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 0", borderBottom: "1px solid var(--border-light)", opacity: n.read ? 0.55 : 1 }}>
            <div style={{ width: 9, height: 9, borderRadius: "50%", marginTop: 5, flexShrink: 0, background: n.read ? "var(--border)" : n.type === "stock" ? "#E24B4A" : "var(--primary)" }} />
            <div style={{ flex: 1 }}>
