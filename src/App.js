...
function Navbar({ cart, wishlist, onCartOpen }) {
  ...
  return (
    <nav className="navbar" style={scrolled ? { boxShadow: "0 2px 20px rgba(176,122,90,0.10)" } : {}}>
      <Link to="/" className="navbar-logo" onClick={() => setMenuOpen(false)}>
        <img src="https://rayfineornates.com/wp-content/uploads/2021/06/logo.png" alt="Ray Fine Ornates" />
      </Link>
      <div className={`nav-links ${menuOpen ? "open" : ""}`}>
        <Link to="/" className={loc.pathname === "/" ? "active" : ""}>Home</Link>
        <Link to="/shop" className={loc.pathname === "/shop" ? "active" : ""}>Shop</Link>
...
function Footer() {
  return (
    <footer className="footer">
      <div className="footer-top">
        🚚 Free Express Delivery &nbsp;·&nbsp; ✨ Use Code <strong>GIFT15</strong> for 15% Off &nbsp;·&nbsp; 🌍 Ships to 140+ Countries
      </div>
      <div className="footer-grid">
        <div>
          <h4>Ray Fine Ornates</h4>
        ...
        <div>
          ...
          <div className="footer-contact-item"><span className="footer-contact-icon">☎</span><span style={{ fontSize: "13px", color: "var(--text-muted)" }}>+91 8112240112</span></div>
